$(() => {
  const $coordinatesElement = $(".scope_coordinates_container");
  ...
  const handleParentAreaScopeChange = (ev) => {
    ...
    $.ajax({
      method: "GET",
      url: updateUrl,
      data: { parent_scope_id: scopeId, setting_name: settingName } // eslint-disable-line
    }).done((data) => {
      $(".scope-coordinates", $coordinatesElement).replaceWith(data);
    });
  };
  ...
  $(document).on(
    "change",
    "input[name='component[settings][scope_id]']",
